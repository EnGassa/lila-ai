...
export function ProductDialog({
  ...
  const defaultOptions =
    product?.product_purchase_options?.map((opt) => ({
      id: opt.id,
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      if (selectedFile) {
        ...
        const signResult = await getSignedUploadUrl(
          fileName,
          selectedFile.type,
        );
      ...
      if (values.review_count !== undefined)
        formData.append("review_count", String(values.review_count));

      if (values.attributes?.length)
        formData.append("attributes", values.attributes.join(", "));
      if (values.benefits?.length)
        formData.append("benefits", values.benefits.join(", "));
      if (values.active_ingredients?.length)
        formData.append(
          "active_ingredients",
          values.active_ingredients.join(", "),
        );
      if (values.concerns?.length)
        formData.append("concerns", values.concerns.join(", "));

      // Append purchase options as JSON string
      if (values.purchaseOptions?.length) {
        formData.append(
          "purchaseOptions",
          JSON.stringify(values.purchaseOptions),
        );
      }
            ...
            <div className="space-y-4 rounded-md border p-4 bg-muted/20">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      Attributes ({KNOWN_ATTRIBUTES.length})
                    </FormLabel>
                    <FormControl>
                      <MultiSelectString
                        value={field.value || []}
                ...
                <FormItem className="flex flex-col gap-2">
                  ...
                  <FormControl>
                    <MultiSelectIngredients
                      ...
                      onChange={(val) =>
                        form.setValue("active_ingredients", val)
                      }
                    />
                  </FormControl>
                  <FormMessage />
            ...
            <div className="space-y-4 rounded-md border p-4 bg-blue-50/50 border-blue-100">
              ...
              <div className="space-y-3">
                {fields.map((field, index) => (
                  <div
                    key={field.id}
                    className="grid grid-cols-12 gap-2 items-start p-2 rounded bg-white border"
                  >
                    <FormField
                      control={form.control}
