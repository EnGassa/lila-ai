...
import { Edit, Loader2, Link as LinkIcon, Trash } from "lucide-react"
...
export function RetailersTable({ retailers: initialRetailers }: RetailersTableProps) {
    ...
    const handleToggleStatus = async (id: string, currentStatus: boolean) => {
         // Optimistic update
         setRetailers(prev => prev.map(r => r.id === id ? { ...r, is_active: !currentStatus } : r))
                    ...
                    <TableBody>
                        {filteredRetailers.map((retailer) => (
                            <TableRow key={retailer.id}>
                                ...
                                <TableCell>
                                    {retailer.base_url ? (
                                        <a href={retailer.base_url} target="_blank" rel="noreferrer" className="flex items-center text-sm text-blue-600 hover:underline">
                                ...
                                <TableCell>
                                    <Switch
                                        ...
                                        onCheckedChange={(checked) => handleToggleStatus(retailer.id, !checked)} // toggle expects current status, logic handled there
                                    />
                                </TableCell>
