...
# Config
# -------------------------
UA = "Bot/1.0 (+https://example.com) Contact: ops@example.com"
...
def parse_jsonld_product(soup: BeautifulSoup) -> Tuple[Optional[str], Optional[str]]:
...
def fallback_name_brand(soup: BeautifulSoup) -> Tuple[Optional[str], Optional[str]]:
    h1 = soup.find("h1")
...
def parse_brand_name_by_ids(soup: BeautifulSoup) -> Tuple[Optional[str], Optional[str]]:
    brand = None
    name = None
...
def merge_ingredients_with_provenance(skim: List[str], overview: List[str], explained: List[str]) -> Tuple[List[str], Dict[str, List[str]]]:
    order: List[str] = []
...
def extract_key_ingredients_from_page(soup: BeautifulSoup) -> List[Dict[str, Optional[str]]]:
...
def augment_with_map(ingredients_inci: List[str], map_json: Optional[Dict[str, str]]) -> List[Dict[str, Optional[str]]]:
...
def dedupe_actives(actives: List[Dict[str, Optional[str]]]) -> List[Dict[str, Optional[str]]]:
    out: List[Dict[str, Optional[str]]] = []
    seen = set()
    for a in actives:
...
def extract_details_blurb(soup: BeautifulSoup, include_html: bool = False) -> Tuple[Optional[str], Optional[str]]:
...
def extract_image_assets(soup: BeautifulSoup) -> Tuple[Optional[str], Optional[str], Dict[str, List[str]]]:
    image_alt = None
    image_url = None
...
def extract_skim_metrics_and_rows(soup: BeautifulSoup) -> Tuple[Optional[int], Optional[int], Optional[int], Optional[int], List[Dict], List[str]]:
    warnings: List[str] = []
    irr_vals: List[int] = []
    com_vals: List[int] = []
...
def guess_category_and_usage(name: str) -> Tuple[Optional[str], Optional[str], Optional[str]]:
    n = (name or "").lower()
...
def parse_brand_anchor_and_slug(soup: BeautifulSoup) -> Tuple[Optional[str], Optional[str], Optional[str]]:
...
def product_slug_from_url(url: str) -> Optional[str]:
    try:
...
def compute_product_id(url: str, brand_slug: Optional[str], product_slug: Optional[str], method: str) -> str:
...
def parse_product_page(html: str, url: str, *, parser: str = DEFAULT_PARSER, map_json: Optional[Dict[str,str]] = None,
                       join_sources: List[str] = None, include_details_html: bool = False,
                       product_id_from: str = "url_hash") -> Tuple[dict, List[Dict]]:
...
def best_rating(a: Optional[str], b: Optional[str]) -> Optional[str]:
...
def upsert_index(index: Dict[str, dict], rows: List[Dict], product_url: str, now_iso: str, cap_sources: int = 10) -> Dict[str, dict]:
    for r in rows:
...
def main():
    ap = argparse.ArgumentParser(description="Scrape an INCIDecoder product page into a JSONL row (provenance ingredients; image assets; optional ingredient index)")
    ...
    ap.add_argument("--map-json", type=str, help="Path to JSON mapping (either {inci: family} or {\"inci_to_family\": {...}})")
    ap.add_argument("--ignore-robots", action="store_true", help="Skip robots.txt check (use only if permitted)")

    ap.add_argument("--join-ingredients-sources", type=str, default="skim,overview,explained", help="Comma-separated sources to join in order (any of: skim,overview,explained)")
    ...
    ap.add_argument("--product-id-from", choices=["url_hash", "slug"], default="slug", help="How to compute product_id (default slug; fallback to url_hash if slugs missing)")

    args = ap.parse_args()

    if not args.ignore_robots and not is_allowed_by_robots(args.url, args.ua):
        sys.stderr.write("Blocked by robots.txt or failed to fetch robots — aborting politely. Use --ignore-robots if you have permission.")
        sys.exit(2)
