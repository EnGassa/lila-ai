...
*   **Secure "Broker" Uploads:** To handle file uploads securely without exposing sensitive credentials or managing complex RLS policies for unauthenticated users, the application uses a "Secure Broker" pattern.
    *   **Client:** The frontend uploads files to a Next.js Server Action (`app/[userId]/upload/actions.ts`). It performs client-side validation and conversion (e.g., HEIC to JPEG).
    *   **Server Action:** The server action acts as a trusted intermediary. It authenticates the `userId`, initializes an S3 client with private admin credentials (`SUPABASE_S3_ACCESS_KEY_ID`), and streams the file securely to a private Supabase bucket (`user-uploads`).
    *   **Storage:** The files are stored in a private bucket, isolated by user ID, ensuring data privacy.
...
*   **"Smart Brute Force" Retrieval Strategy:** To improve the quality of product candidates, the retrieval process was upgraded from a simple semantic search to a more targeted approach.
    *   **Strategy:** The system iterates through every product category ("Brute Force") and, for each one, constructs a highly specific vector search query that includes the user's skin profile, the philosophy's primary goals, and, most importantly, the list of `key_ingredients_to_target` ("Smart").
    *   **Impact:** This ensures that the product pool passed to the Generator agent is not only balanced across all necessary product types but is also pre-filtered to be highly relevant to the specific active ingredients required by the strategic philosophy. This resolves previous issues where critical product types were missing from the candidate list.
*   **"Full Transparency" End-to-End Tracing:** To enhance the debuggability and reliability of the multi-agent system, a comprehensive tracing mechanism has been implemented by adding dedicated "reasoning" fields to the Pydantic models.
    *   **`diagnosis_rationale` (in `SkincarePhilosophy`):** The Strategist agent now articulates *why* it chose specific goals and ingredients, providing a clear link between the user's analysis and the high-level plan.
    *   **`reasoning` (in `Recommendations`):** The Generator agent outputs a step-by-step thought process, detailing how it selected products, constructed the routine, and verified safety rules.
    *   **`audit_log` (in `ReviewResult`):** The Reviewer agent provides a checklist-style log of the specific safety and consistency checks it performed, confirming its validation process. This log is printed to the console upon successful validation.
    *   **Benefit:** This end-to-end trace provides a "flight recorder" for the entire AI decision-making process, making it significantly easier to debug, validate, and trust the generated outputs.
