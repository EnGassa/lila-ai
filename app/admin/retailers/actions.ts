...
export async function createRetailer(
  ...
  if (error) {
    if (error.code === "23505")
      return { success: false, message: "Retailer name must be unique." };
...
export async function toggleRetailerStatus(
  id: string,
...
export async function deleteRetailer(id: string): Promise<ActionState> {
  ...
  if (error) {
    if (error.code === "23503")
      return {
        success: false,
        message: "Cannot delete retailer because it is linked to products.",
      };
    return { success: false, message: error.message };
  }

  revalidatePath("/admin/retailers");
  return { success: true, message: "Retailer deleted" };
}
